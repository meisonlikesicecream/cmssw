...
#include "TMTrackTrigger/TMTrackFinder/interface/Get3Dtracks.h"
...
void Histos::fillTrackCands(const InputData& inputData, const matrix<Get3Dtracks> mGet3Dtrks, bool withRZfilter) {
  ...
  // Plot number of stubs per output HT opto-link.
	...
	unsigned int nTrk = trks.size(); 
	  ...
	  tpRecoedInEtaSec = true;    // This TP was reconstructed at least once in this eta sector.
  	  ...
  	  nTrksMatchingTPs += nTrk;   // Increment sum by no. of tracks this TP was reconstructed as
          profDupTracksVsEta_[tName]->Fill(fabs(tp.eta()), nTrk); // Study duplication of tracks within an individual HT array.
          profDupTracksVsInvPt_[tName]->Fill(fabs(tp.qOverPt()), nTrk); // Study duplication of tracks within an individual HT array.
        }
      }
...
void Histos::bookTrackFitting() {
  ...
  for (const string& fitName : trackFitters_ ) {
    ...
    profDupFitTrksVsInvPt_[fitName] = inputDir.make<TProfile>(addn("DupFitTrksVsInvPt") ,"; 1/Pt; No. of duplicate tracks per TP",16,0.,maxAbsQoverPt);
...
void Histos::fillTrackFitting( const InputData& inputData, const map<string,vector<L1fittedTrack>>& mFittedTracks) {  
  ...
  for (const string& fitName : trackFitters_) {
    ...
    for (const TP& tp: vTPs) {
      if (tpRecoedMap[&tp]) { // Was this truth particle successfully fitted?
	profDupFitTrksVsEta_[fitName]->Fill(fabs(tp.eta()), tpRecoedDup[&tp]); 
	profDupFitTrksVsInvPt_[fitName]->Fill(fabs(tp.qOverPt()), tpRecoedDup[&tp]); 
      }
    }
