...
#options.register('trkFitAlgo','All',VarParsing.VarParsing.multiplicity.singleton,
options.register('trkFitAlgo','KF4ParamsComb',VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.string,"Nuame of track helix fit algorithm")
...
if 'All' in options.trkFitAlgo:
  options.trkFitAlgo = ['KF4ParamsComb', 'SimpleLR', 'Tracklet']
...
# Save CPU by skipping MC truth & Histos (OK if only care about output TTTrack collection).
...
if 'KF4ParamsComb' in options.trkFitAlgo:
  process.TMTrackProducer.TrackFitSettings.TrackFitters = cms.vstring("KF4ParamsComb")
...
if 'SimpleLR' in options.trkFitAlgo:
  process.TMTrackProducerwithSF = process.TMTrackProducer.clone()
  process.TMTrackProducerwithSF.TrackFitSettings.TrackFitters = cms.vstring("SimpleLR")
  process.TMTrackProducerwithSF.RZfilterOpts.UseSeedFilter    = cms.bool(True)

  process.TTAssociatorTMTTSimpleLR = process.TTTrackAssociatorFromPixelDigis.clone(
          TTTracks = cms.VInputTag(cms.InputTag("TMTrackProducerwithSF", 'TML1TracksSimpleLR'))
      )
  process.L1TrackNtuple_TMTT_SimpleLR = process.L1TrackNtuple.clone(
          L1TrackInputTag = cms.InputTag("TMTrackProducerwithSF", 'TML1TracksSimpleLR'),
          MCTruthTrackInputTag = cms.InputTag("TTAssociatorTMTTSimpleLR", 'TML1TracksSimpleLR'),
        )
  process.p_TMTT_SimpleLR = cms.Path( process.TMTrackProducerwithSF*process.TTAssociatorTMTTSimpleLR*process.L1TrackNtuple_TMTT_SimpleLR )
